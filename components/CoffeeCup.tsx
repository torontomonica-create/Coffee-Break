...
export const CoffeeCup: React.FC<CoffeeCupProps> = ({ type, sipsTaken, onSip }) => {
  ...
  const playSipSound = () => {
    try {
      if (!audioRef.current) {
        audioRef.current = new Audio('/sip.mp3');
      }
  ...
  return (
    <div className="relative flex flex-col items-center justify-center h-auto w-full max-w-[280px] sm:max-w-[320px] mx-auto select-none p-3 sm:p-4">
      ...
      {/* Sip Instruction */}
      <div className={`relative z-10 mb-6 sm:mb-10 text-stone-400 font-bold tracking-wide text-base sm:text-xl animate-pulse text-center w-full transition-opacity duration-500 ${remainingSips > 0 ? 'opacity-100' : 'opacity-0'}`}>
        Tap or Hold to Sip!
      </div>
